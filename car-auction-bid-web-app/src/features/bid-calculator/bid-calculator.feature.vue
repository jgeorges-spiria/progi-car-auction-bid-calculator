...
async function fetchBidCalculation(
  vehiclePrice: string,
  ...
  const newBid = await CarAuctionBidApiService.calculateBid(
    vehiclePrice,
    vehicleType,
  );
  if (newBid) {
...
<template>
  ...
  <FailedBidCalculationErrorBanner
    :show-banner="showFailedBidCalculationError"
    @hide="hideFailedBidCalculatorErrorMessage"
  />
  <div class="card">
